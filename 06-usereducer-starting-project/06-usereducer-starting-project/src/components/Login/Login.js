...
const reducerFnTwo = (state, action) => {
  if (action.type === "USER_INPUT_PASSWORD")
    return { value: action.val, isValid: action.val.includes("@") };
  if (action.type === "USER_BLUE_PASSWORD")
...
const Login = (props) => {
  ...
  const [passwordState, dispatchPassword] = useReducer(reducerFnTwo, {
    value: "",
    isValid: false,
  });
  useEffect(() => {
  ...
  const validatePasswordHandler = () => {
    dispatchPassword({ type: "USER_BLUE_PASSWORD" });
